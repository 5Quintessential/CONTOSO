...
namespace Microsoft.DPE.ReferenceApps.Food.Lib.Services.BingMaps
{
    ...
    {
        string BingKey;
        public SearchHelper(string bingKey)
        {
            BingKey = bingKey;
        }
        ...
        public static async Task<List<Resource>> FindLocationByPointAsync(double latitude, double longitude)
        {
            ...
            var _Url = string.Format(_Template, latitude, longitude, Settings.BingMapsKey);
        ...
        public static async Task<List<Resource>> FindLocationByQueryAsync(string query, int maxResults = 5)
        {
            ...
            var _Url = string.Format(_Template, query, maxResults, Settings.BingMapsKey);
            var _Result = await CallService(new Uri(_Url));
            if (_Result == null)
                return null;
            return _Result.resourceSets[0].resources;
        ...
        public async Task<List<Resource>> FindLocationByAddressAsync(Countries country = Countries.US, string adminDistrict = null, string locality = null, string postalCode = null, string addressLine = null, string userLocation = null, string userIP = null, int maxResults = 5)
        {
            ...
            var _Url = string.Format(_Template, country.ToString(), adminDistrict, locality, postalCode, addressLine, userLocation, userIP, maxResults, Settings.BingMapsKey);
            var _Result = await CallService(new Uri(_Url));
            if (_Result == null)
                return null;
            return _Result.resourceSets[0].resources;
